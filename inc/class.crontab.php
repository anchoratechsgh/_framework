...
Class crontab_manager {
    ...
    function __construct($host=NULL, $port=NULL, $username=NULL, $password=NULL) {
	    ...
	    try {
				if (is_null($host) || is_null($port) || is_null($username) || is_null($password)) $this->error_message("Please specify the host, port, username and password!");
	        $this->connection = @ssh2_connect($host, $port);
	        if ( ! $this->connection) $this->error_message("The SSH2 connection could not be established.");
	        $authentication = @ssh2_auth_password($this->connection, $username, $password);
	        if ( ! $authentication) $this->error_message("Could not authenticate '{$username}' using password: '{$password}'.");
	    } catch (Exception $e) {
