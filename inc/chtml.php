...
final class CHtml
{
    ...
    public static function meta($metaOptions = array())
    {
        ...
        if (array_key_exists('title', $metaOptions)) {
            echo '<title>' . $metaOptions['title'] . '</title>' . "\n";
        ...
        if (array_key_exists('author', $metaOptions)) {
            echo '<link rel="author" href="' . $metaOptions['author'] . '" />' . "\n";
        }
    ...
    public static function printFrame($path, $id = 'printframe', $height = 700, $width = 100)
    {
        return '<iframe allowtransparency="1" frameborder="0" src="' . $path . '" id="' . $id . '" name="' . $id . '" style="border-radius:4px;width:' . $width . '%; height:' . $height . 'px; border:1px #000000 solid; padding:20px 10px 30px 10px;"></iframe>';
    }
